...
function [P, it, res] = MW(P, RHS, h, epsi, itermax, omg, h1)
...
if (h < h1) && (mod(jmax, 2) == 0) && (mod(imax, 2) == 0)
    ...
    % first recursive call to MW
    u2h = MW(zeros(size(f2h)), f2h, 2*h, epsi, itermax, omg, h1);
    
    % second recursive call to MW
    u2h = MW(u2h, f2h, 2*h, epsi, itermax, omg, h1);
...
%% post-smoothing
[P, it, res] = SOR(P, RHS, 0, 0, 0, 0, imax, jmax, h, h, epsi, itermax, omg, 0);
